...
def heal_check(action, healer, target):
    ...
    if "Heal" in action.effects:
        ...
        # TODO: Make this a separate mini-function for this function and CmdAttack. Combat_log_entry will change too.
        if healer.db.final_action:
            healer.db.final_action = False
            healer.db.KOed = True
            healer.msg("You have taken your final action and can no longer fight.")
            combat_string = "|y<COMBAT>|n {0} can no longer fight.".format(healer.name)
            healer.location.msg_contents(combat_string)
            combat_log_entry(healer, combat_string)
        return True
    else:
        return False